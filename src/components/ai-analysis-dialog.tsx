  ...
  useEffect(() => {
    if (isOpen && !analysis && !isLoading) {
      ...
      getAIMatchAnalysis({ userProfile1: user1, userProfile2: user2 })
        .then(setAnalysis)
        .catch(() => setError('Failed to get AI analysis. Please try again.'))
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px] bg-card border-primary/20">
        <DialogHeader>
          ...
          <DialogDescription>
            Our AI's thoughts on you and {user2.name}.
          </DialogDescription>
        </DialogHeader>
        <div className="py-4 space-y-6">
          {isLoading && (
            <div className="flex flex-col items-center justify-center space-y-4 h-64">
              <p className="text-muted-foreground animate-pulse">Analyzing compatibility...</p>
            </div>
          )}
          ...
          {analysis && (
            <div className="space-y-4">
              ...
              {analysis.sharedHobbies.length > 0 && (
                <div>
                  <h3 className="font-semibold flex items-center gap-2"><Users className="text-primary"/> Shared Hobbies</h3>
              ...
              {analysis.sharedInterests.length > 0 && (
                <div>
                    <h3 className="font-semibold flex items-center gap-2"><Heart className="text-primary"/> Shared Interests</h3>
                    <div className="flex flex-wrap gap-2 mt-2">
              ...
              <div>
                <h3 className="font-semibold flex items-center gap-2"><Star className="text-primary"/> AI Verdict</h3>
